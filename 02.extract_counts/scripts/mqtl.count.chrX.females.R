...
#k=1
path=paste("/panfs/panasas01/shared-godmc/sftp/GoDMC/",users[k],"/",cohorts[k],"/results/14/",sep="")
...
write.table(eqtl2,paste("/panfs/panasas01/shared-godmc/meta-analysis/inputfiles/14/",users[k],"_",cohorts[k],".",i,".gwama.formatted.txt",sep=""),sep="\t",quote=F,row.names=F,col.names=F)
...
write.table(assoc.cis[[p]],paste("/panfs/panasas01/shared-godmc/counts/14/",users[k],"_",cohorts[k],"/cis.assoc.",i,".",users[k],"_",cohorts[k],".",pvals[p],".txt",sep=""),quote=F,row.names=F,col.names=F,sep="\t")
write.table(assoc.trans[[p]],paste("/panfs/panasas01/shared-godmc/counts/14/",users[k],"_",cohorts[k],"/trans.assoc.",i,".",users[k],"_",cohorts[k],".",pvals[p],".txt",sep=""),quote=F,row.names=F,col.names=F,sep="\t")
...
write.table(eqtl.count,paste("/panfs/panasas01/shared-godmc/cohort_summary/mqtlcount.",users[k],"_",cohorts[k],"_chrX_females.txt",sep=""),quote=F,row.names=F,col.names=T,sep="\t")
...
eqtl.count<-read.table(paste("/panfs/panasas01/shared-godmc/cohort_summary/mqtlcount.",users[k],"_",cohorts[k],"_chrX_females.txt",sep=""),header=T)
...
write.table(out,paste("/panfs/panasas01/shared-godmc/cohort_summary/mqtlcount.",users[k],"_",cohorts[k],"_chrX_females.summary.txt",sep=""),quote=F,row.names=F,col.names=T,sep="\t")
}
